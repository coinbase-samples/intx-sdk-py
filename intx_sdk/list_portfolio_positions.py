...
@dataclass
class GetPortfolioPositionsRequest:
    portfolio: str
    allowed_status_codes: Optional[List[int]] = None


@dataclass
class GetPortfolioPositionsResponse(BaseResponse):
...
class IntxClient:
    ...
    def get_portfolio_positions(self, request: GetPortfolioPositionsRequest) -> GetPortfolioPositionsResponse:
        path = f"/portfolios/{request.portfolio}/positions"
        response = self.client.request("GET", path, allowed_status_codes=request.allowed_status_codes)
        return GetPortfolioPositionsResponse(response.json(), request)
