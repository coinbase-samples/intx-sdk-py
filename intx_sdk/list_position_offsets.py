...
@dataclass
class ListPositionOffsetsRequest:
    allowed_status_codes: Optional[List[int]] = None


@dataclass
class ListPositionOffsetsResponse(BaseResponse):
...
class IntxClient:
    ...
    def list_position_offsets(self, request: ListPositionOffsetsRequest) -> ListPositionOffsetsResponse:
        path = f"/position-offsets"

        response = self.client.request("GET", path, allowed_status_codes=request.allowed_status_codes)
        return ListPositionOffsetsResponse(response.json(), request)
