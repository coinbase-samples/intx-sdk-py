...
@dataclass
class GetBalanceForPortfolioAssetRequest:
    portfolio: str
    asset: str
...
@dataclass
class GetBalanceForPortfolioAssetResponse(BaseResponse):
...
class IntxClient:
    ...
    def get_balance_for_portfolio_asset(
            self,
            request: GetBalanceForPortfolioAssetRequest) -> GetBalanceForPortfolioAssetResponse:
        path = f"/portfolios/{request.portfolio}/balances/{request.asset}"
        response = self.client.request("GET", path, allowed_status_codes=request.allowed_status_codes)
        return GetBalanceForPortfolioAssetResponse(response.json(), request)
