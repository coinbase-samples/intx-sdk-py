...
@dataclass
class EnableDisableCrossCollateralResponse(BaseResponse):
...
class IntxClient:
    ...
    def enable_disable_cross_collateral(self,
                                        request: EnableDisableCrossCollateralRequest) -> EnableDisableCrossCollateralResponse:
        path = f"/portfolios/{request.portfolio}/cross-collateral-enabled"
        body = {"enabled": request.enabled}
        response = self.client.request("POST", path, body=body, allowed_status_codes=request.allowed_status_codes)
        return EnableDisableCrossCollateralResponse(response.json(), request)
